...
오늘 첫 출근한 신입 작가가 되어, 생방송 10분 전 야외 세트에 흩어진 **사연 카드 다섯 장**을
찾아 부스로 복귀하는 게임입니다.
...
- **전투 요소가 전혀 없습니다.** 원본 엔진은 1인칭 슈터였지만, 실존 인물이나 사람을 공격 대상으로
  삼는 형태는 의도적으로 전부 제거했습니다. 무기는 숨겨졌고, 적 캐릭터는 한 명도 생성되지 않으며,
  사격 입력 자체가 차단되어 있습니다.
...
|---|---|---|
| 장르 | 1인칭 슈터 | 비전투 내러티브 |
| 적 캐릭터 | 14명 | **0명** (`Config.ai.count = 0`) |
| 무기 | M4 카빈 뷰모델 | **숨김** (`viewmodelScene.visible = false`) |
| 사격 | 마우스 좌클릭 | **입력 차단** (포인터 락 상태의 클릭을 캡처 단계에서 삼킴) |
...
1. **전투 제거** — 무기 뷰모델 숨김, 원본 HUD 숨김, 사격 입력 차단
2. **한글 HUD** — 목표 / 방송까지 남은 시간 / 카드 수집 현황
